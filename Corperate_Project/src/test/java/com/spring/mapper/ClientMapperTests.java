...
public class ClientMapperTests {
	...
	private ClientMapper mapper; // 정의해둔 인터페이스로 자동 생성된 적절한 객체 -> 이 객체를 이용해 xml에 존재하는 SQL문을 실행 혹은 @Select 같은 어노테이션 SQL문 실행
	...
	public void testGetListWithPaging1() {
		...
		List<ClientVO> clients = mapper.getListWithPaging(cri);
		clients.forEach(client -> log.info(client));
	}
	
	@Test
	public void testGetListWithPaging2() {
		Criteria cri = new Criteria(1,10,"name","test");
		List<ClientVO> clients = mapper.getListWithPaging(cri);
		clients.forEach(client -> log.info(client));
	}
	...
	public void testGetListWithPaging3() {
		Criteria cri = new Criteria(1,10,"bank","한국시티");
		List<ClientVO> clients = mapper.getListWithPaging(cri);
		clients.forEach(client -> log.info(client));
	}

	@Test
	public void testGetCountAll1() {
		Criteria cri = new Criteria();
